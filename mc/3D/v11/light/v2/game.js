// ============================================================================
// MATTYOU CRAFT - MAIN GAME ENGINE (game.js) - LIGHT EDITION
// ============================================================================
...
class Game {
    ...
    initEngine() {
        ...
        this.renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2)); // Limiteer pixel ratio voor betere performance op retina schermen
    ...
    initMouseAndScrollTracking() {
        // Muis cursor tracking voor vastgehouden item in de inventaris
        ...
        // Scrollwiel support voor hotbar selectie
    ...
    updateChunksAroundPlayer() { this.worldManager.updateChunksAroundPlayer(this.player.position, this.activeWorld.renderDistance || 2); }
    ...
    performPlacingOrInteraction() {
        ...
        if (intersects.length > 0 && intersects[0].distance < 5.0) {
            ...
            // Als we op een Crafting Table klikken, open de 3x3 grid!
            if (clickedBlockId === 7) {
            ...
            // Voorkom dat de speler een blok in zichzelf plaatst
            const pBox = {
    ...
    updateCraftingOutput() {
        ...
        // 1. FLEXIBEL: Houten Planken (1 Hout Stam (ID 4) ergens in de grid)
        ...
        // 2. FLEXIBEL: Stokken (2 Planken (ID 6) verticaal boven elkaar)
        if (!matched) {
        ...
        // 3. FLEXIBEL: Crafting Table (4 Planken (ID 6) in een 2x2 vierkant)
        if (!matched) {
            const plankCount = gridIds.filter(id => id === 6).length;
            const otherCountPlank = gridIds.filter(id => id !== null && id !== 6).length;
    ...
    createAndStartWorld() {
        ...
        const renderDistance = parseInt(document.getElementById('world-render-distance').value) || 2;
    ...
    loadWorld(world) {
        ...
        // Vul de speler inventaris op basis van de gekozen modus
