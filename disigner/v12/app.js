...
function createBaseObjectData(mesh, nameType, extraData = {}) {
    objectCounter++;
    // Cruciaal: Behoud het bestaande ID bij deserialisatie om mismatches te voorkomen!
...
function saveSelectionAsModel() {
    ...
    // Verzamel het geselecteerde object en alle hiërarchische sub-objecten
    const collected = [];
...
function spawnModel(modelName) {
    ...
    // Map oude IDs naar nieuwe unieke IDs om conflicten te voorkomen
    const idMap = {};
    ...
    collectedData.forEach(data => {
        ...
        // Verschuif de positie iets zodat ze niet exact op elkaar spawnen
        ...
        const objData = {
            ...
            keyframes: JSON.parse(JSON.stringify(data.keyframes)), // diepe kopie van keyframes
    ...
    // Herstel de Three.js hiërarchie
    ...
    // Selecteer het hoofd-gespawnde object
...
function deserializeObject(data) {
    ...
    // Geef alle opgeslagen data direct door om ID-mismatch te voorkomen!
...
function loadProject(name) {
    ...
    if (!raw) {
        ...
        // Zorg dat de defaults altijd goed staan als er geen data is!
        skyColor = '#0f172a';
        groundColor = '#22c55e';
        showGrid = true;
        exportGrid = false;
