...
function initThree() {
    ...
    transformControls.addEventListener('change', () => {
        if (transformControls.object) {
            // AUTO-KEYFRAME VOOR ALLE GESELECTEERDE OBJECTEN:
            // Dit voorkomt dat andere geselecteerde objecten ongewenst terugspringen!
...
function addObject(type) {
    ...
    if (type === '3dtext') {
        ...
        // Gebruik MeshBasicMaterial zodat de tekst altijd perfect verlicht en zichtbaar is!
...
function create3DTextTexture(text, colorHex) {
    ...
    // Mooie semi-transparante achtergrond zodat de tekst altijd leesbaar is
...
function saveScreenButtonData() {
    ...
    const btnData = {
        ...
        // Standaard posities in het midden-onderen
...
function updateScreenButtonsUI() {
    ...
    // Update live preview sleepbare knoppen op het canvas
    ...
    screenButtons.forEach(b => {
        ...
        // Positioneer op basis van opgeslagen percentages
        btn.style.left = `${b.left}%`;
        btn.style.top = `${b.top}%`;

        // Sleep functionaliteit (Drag & Drop)
        ...
        // Klik actie (alleen triggeren als we niet gesleept hebben)
...
function updateSelectedObjectFromInputs() {
    ...
    // AUTO-KEYFRAME: Sla direct op bij handmatige invoer om resets te voorkomen!
...
<body>
    ...
    <script>
        ...
        // 1. Maak alle meshes aan en sla ze op in een map
        ...
        // 2. Reconstrueer de hiërarchie (voeg kinderen toe aan hun parent groep, anders aan de scene)
        ...
        let isPlaying = true; // Standaard speelt de animatie af
        ...
        window.handleScreenBtnClick = function(type, target) {
            if (type === 'open_menu') openMenu(target);
                ...
                isPlaying = !isPlaying; // Start / Pauzeer de animatie via de knop!
            }
        }
